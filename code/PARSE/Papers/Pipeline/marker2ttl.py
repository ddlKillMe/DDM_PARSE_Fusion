...
_auth_pat = re.compile(r"(?:^|\W)([A-Z][A-Za-z\-]+)(?:\s+et\s+al\.)?(?:\s+and\s+[A-Z][A-Za-z\-]+)?\s*(?:,|\(|\s)(\d{4})(?:\)|\b)")
...
def build_xml(md:str)->ET.Element:
    ...
    for sec in parse_markdown_structure(md):
        sec_el = ET.SubElement(root,"section",ID=str(sec["index"]),index=str(sec["index"]),level=str(sec["level"]))
        ...
        for pi,para in enumerate(split_paragraphs(sec["content"]),1):
            ...
            for si,s in enumerate(split_sentences(para),1):
                sent_el = ET.SubElement(para_el,"sentence",ID=f"{sec['index']}.{pi}.{si}",index=str(si))
...
def generate_ttl(xml_root:ET.Element, md:str, out:str, paper_id:str, existing):
    g = Graph()
    g.bind("askg-data",ASKG_DATA); g.bind("askg-onto",ASKG_ONTO); g.bind("domo",DOMO)
    index_p=URIRef(ASKG_ONTO+"index"); level_p=URIRef(ASKG_ONTO+"level")
    n_sent_p=URIRef(ASKG_ONTO+NUMBER_OF_SENTENCES); has_cit_p=URIRef(ASKG_ONTO+HAS_CITATION)

    paper_uri = URIRef(ASKG_DATA+f"Paper-{clean_uri(paper_id)}")
    paper_title = next((ln[2:].strip() for ln in md.splitlines() if ln.startswith("# ")),paper_id)
    ...
    for sec in xml_root.findall("./section"):
        ...
        g.add((sec_uri,n_sent_p,Literal(sec.get("numberOfSentences"),datatype=XSD.positiveInteger)))
        hd=sec.find("heading")
        if hd is not None: g.add((sec_uri,DOMO.Text,Literal(hd.text,lang="en")))
        sec_cit:set[str]=set()

        for para in sec.findall("paragraph"):
            para_id=para.get("ID")
            para_uri=URIRef(ASKG_DATA+f"Paper-{clean_uri(paper_id)}-Section-{sec_id}-Paragraph-{clean_uri(para_id)}")
            ...
            g.add((para_uri,n_sent_p,Literal(para.get("numberOfSentences"),datatype=XSD.positiveInteger)))
            txt=para.find("text")
            if txt is not None: g.add((para_uri,DOMO.Text,Literal(txt.text,lang="en")))
            ...
            for sent in para.findall("sentence"):
                ...
                for mark in extract_citations(stext):
                    ...
                    if lit not in para_cit:
                        g.add((sent_uri,has_cit_p,Literal(lit))); para_cit.add(lit)
                    if lit not in sec_cit:
                        g.add((para_uri,has_cit_p,Literal(lit))); sec_cit.add(lit)
            for lit in para_cit:
                if lit not in sec_cit:
                    g.add((sec_uri,has_cit_p,Literal(lit))); sec_cit.add(lit)

    g.serialize(out,format="turtle"); print("✓ TTL saved →",out)
